...
Avoid these when prompting the Cursor agent.
...
```text
Use your GPT-5 reasoning mode and answer like Claude would.
```

Better:
- Write one model-agnostic prompt with explicit contracts.
- Let Cursor route the run; encode effort only through the model id or bracket parameters when the user asked for it.
