...
Reviews run read-only via Cursor's plan mode: the reviewer can read files in the working tree but cannot execute shell commands or make changes.
...
- `/cursor:review` is fixed-scope review only. It does not support staged-only review, unstaged-only review, or extra focus text.
